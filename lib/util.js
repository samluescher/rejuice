 ...
 */
var getHistogramDimensions = (function(min, max, desiredBins) {
    var deltaOrderOfMagniture = Math.max(1, Math.round(Math.log10(max - min))),
        numBins = Math.max(1, Math.round(
		    (max - min) / Math.pow(10, deltaOrderOfMagniture)
            * desiredBins));
    return {
        min: min,
        max: max,
